...
from .models import PartiesEvent
from .models import WeddingEvent
from .models import CollegeEvent
from .models import EventAdvisor
...
def technicalevent(request):
    if request.method == "POST":
        ...
        technicalevent = TechnicalEvent(name=name, email=email, phone=phone, destination=destination,date = date,time = time,guest = guest, desc =desc)
        technicalevent.save()
        print(technicalevent)
        return render(request,'payment.html')
